...
class FlxAnimateFrames extends FlxAtlasFrames
{
	...
	public static function fromTextureAtlas(Path:String):FlxAnimateFrames
	{
		...
		if (FlxAnimate.assetsExists('$Path/spritemap.json'))
		{
		...
		while (!isDone)
		{
			if (FlxAnimate.assetsExists('$Path/spritemap$i.json'))
	...
	public static function fromSpriteMap(Path:FlxSpriteMap, ?Image:FlxGraphicAsset):FlxAtlasFrames
	{
		...
		if (Path is String)
		{
			...
			var text = (StringTools.contains(str, "/")) ? FlxAnimate.assetsGetText(str) : str;
	...
	{
		if (Path is String && !FlxAnimate.assetsExists(Path))
			return null;

		var data:Xml = (Path is String) ? Xml.parse(FlxAnimate.assetsGetText(Path)).firstElement() : Path.firstElement();
	...
	public static function fromJson(Path:FlxJson, ?Image:FlxGraphicAsset):FlxAtlasFrames
	{
		if (Path is String && !FlxAnimate.assetsExists(Path))
			...
			return null;
		var data:JsonNormal = (Path is String) ? haxe.Json.parse(FlxAnimate.assetsGetText(Path)) : Path;
	...
	public static function fromSequence(Path:String, ?ext:String = "png")
	{
		...
		while (FlxAnimate.assetsExists(Path + Std.string(10000 + i).substring(1) + '.$ext'))
		{
	...
	{
		if (!FlxAnimate.assetsExists(Path))
			return null;
		var data:Plist = PropertyList.parse(FlxAnimate.assetsGetText(Path));
	...
	public static function fromEaselJS(Path:String, ?Image:FlxGraphicAsset):FlxAnimateFrames
	{
		...
		var separatedJS = FlxAnimate.assetsGetText(Path).split("\n");
		...
		for (i in 0...names.length)
		{
			...
			var name = names[i];
			var json = jsons[i];
			var bitmap = FlxG.bitmap.add(FlxAnimate.assetsGetBitmapData((Image == null) ? '${imagePath.join("/")}/${json.images[0]}' : Image));
	...
	public static function findImage(Image:FlxGraphicAsset, Path:String):{crash:Bool, ?graphic:FlxGraphic, ?frames:FlxAtlasFrames}
	{
		...
		if (Image is String)
		{
			final temp:String = Image;
			if (FlxAnimate.assetsExists(temp)) Image = FlxAnimate.assetsGetBitmapData(temp);	
		}
